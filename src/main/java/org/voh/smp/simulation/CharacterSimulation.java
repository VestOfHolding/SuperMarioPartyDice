    ...
    public void run() {
        ...
        for (int i = 0; i < SIM_COUNT; ++i) {
            ...
            if (probe) {
                ...
                if (i < SIM_COUNT / 2) { firstHalf += coins; n1++; }
                else                   { secondHalf += coins; n2++; }
                if (i % 50_000 == 49_999)
                    System.out.printf("  [%s] MARIO games=%d running coin mean=%.3f%n",
                            gameBoard.getClass().getSimpleName(), i + 1, (firstHalf + secondHalf) / (i + 1));
            }
            simulationStatTracker.endGame();
            gameBoard.resetBoard();
        }
        if (probe) {
            System.out.printf("  [%s] MARIO firstHalf=%.3f secondHalf=%.3f Δ=%.3f%n",
                    gameBoard.getClass().getSimpleName(), firstHalf / n1, secondHalf / n2, firstHalf / n1 - secondHalf / n2);
        }
